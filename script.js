...
const dom = {
    ...
    sidebarOpenBtn: document.getElementById('sidebarOpenBtn'), // Tombol buka sidebar di header
    sidebarCloseBtn: document.getElementById('sidebarCloseBtn'), // Tombol tutup sidebar di dalam sidebar
...
function setupSidebarToggle() {
    ...
    const applySidebarState = () => {
        ...
        if (isMobile) {
            // Perilaku mobile: sidebar menutupi konten
            dom.sidebar.classList.add('w-full');
            dom.sidebar.classList.remove('w-64');
            dom.mainContent.classList.remove('ml-64'); // Konten utama selalu di ml-0 di mobile
            ...
            dom.sidebar.classList.remove('w-full');
            dom.sidebar.classList.add('translate-x-0'); // Sidebar selalu terbuka di desktop
            dom.sidebar.classList.remove('-translate-x-full');
...
function showHomePage() {
    ...
    dom.mainElement.innerHTML = '<div id="cardsContainer" class="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 md:gap-6"></div>';
    dom.cardsContainer = document.getElementById('cardsContainer'); // Perbarui referensi setelah innerHTML

    // Cek apakah ada filter yang aktif
    const areFiltersActive = Object.values(state.filters).some(filterValue => filterValue !== '');

    // Jika tidak ada filter aktif DAN sudah ada kartu di state, tampilkan langsung
    if (state.cards.length > 0 && !areFiltersActive) {
        displayCards(false); // Tampilkan kartu yang sudah ada, jangan reset
    } else {
        // Jika ada filter aktif atau belum ada kartu, muat ulang kartu
        loadCards(true); 
    }
}
...
function renderCardDetailPage() {
    dom.mainElement.innerHTML = `
        <div class="w-full">
            <button id="backToHome" class="mb-6 px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 transition-colors duration-200">← Kembali ke Kartu</button>
                ...
                <div class="flex flex-col lg:flex-row gap-6 md:gap-8">
                    <div class="lg:w-auto flex-shrink-0 space-y-4">
                        ...
                        <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4">
                            <button id="downloadImageBtn" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition-colors duration-200 text-sm">Unduh Gambar</button>
                            <button id="downloadCroppedBtn" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition-colors duration-200 text-sm">Unduh Terpotong</button>
                        </div>
                    </div>
    ...
    if (isGridView) {
        ...
        function populateCardDetail() {
            ...
            const createDetailBox = (label, value, filterType = null) => { 
                ...
                const clickableClass = filterType ? 'clickable-detail-filter cursor-pointer hover:bg-gray-300 transition-colors duration-200' : '';
        ...
        function displayCards(reset = true) {
            ...
            // Mengatur cardsContainer untuk minimal 3 kolom pada semua ukuran layar
            dom.cardsContainer.className = isGrid ? 'grid grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 md:gap-6' : 'space-y-4';
        ...
        function switchView(view) {
            ...
            // Menyesuaikan ukuran tombol dan menghilangkan emoji
            dom.gridViewBtn.className = `px-2 py-1 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 transition-colors duration-200 text-sm`;
            dom.listViewBtn.className = `px-2 py-1 bg-gray-200 text-gray-800 rounded hover:bg-gray-300 transition-colors duration-200 text-sm`;
            displayCards();
        }
        ...
        function loadVariantArtwork(card) {
            ...
            variantContainer.innerHTML = card.card_images.map((image, i) => 
                `<div class="variant-artwork cursor-pointer hover:opacity-75 transition-opacity duration-200" data-image-index="${i}">
        ...
        const showLoading = show => {
            if (show) {
                ...
                setTimeout(() => dom.loadingIndicator.classList.remove('opacity-0'), 10); // Sedikit delay untuk transisi
            } else {
                dom.loadingIndicator.classList.add('opacity-0');
                dom.loadingIndicator.addEventListener('transitionend', () => {
                    if (dom.loadingIndicator.classList.contains('opacity-0')) {
